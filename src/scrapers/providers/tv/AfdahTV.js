...
async function AfdahTV(req, sse) {
    ...
    //             });
            ...
            if (!sse.stopExecution) {
                logger.error({source: 'AfdahTV', sourceUrl: url, query: {title: req.query.title, season: req.query.season, episode: req.query.episode}, error: err.message || err.toString()});
            }
        }
